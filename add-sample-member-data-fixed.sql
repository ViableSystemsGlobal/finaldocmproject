-- Add sample member details data to existing tables (TRULY SAFE VERSION)
-- Run this in your Supabase Dashboard > SQL Editor
-- This version uses dynamic SQL to avoid parsing issues with non-existent tables
...
BEGIN
    RAISE NOTICE 'Starting member data population...';
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'event_attendance') THEN
        ...
        RAISE NOTICE 'Cleared existing attendance data';
    ELSE
        RAISE NOTICE 'event_attendance table does not exist - will skip attendance data';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'donations') THEN
        ...
        RAISE NOTICE 'Cleared existing donations data';
    ELSE
        RAISE NOTICE 'donations table does not exist - will skip donations data';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'member_notes') THEN
        ...
        RAISE NOTICE 'Cleared existing member notes data';
    ELSE
        RAISE NOTICE 'member_notes table does not exist - will skip member notes data';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'events' AND column_name = 'title') 
       AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'events' AND column_name = 'start_date') THEN
        ...
        RAISE NOTICE 'Added/verified events exist';
    ELSE
        RAISE NOTICE 'Events table missing title or start_date columns. Run fix-member-details-tables.sql first.';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'event_attendance') THEN
        ...
        WHERE contact_id IN (contact1_id, contact2_id);
        
        RAISE NOTICE 'Added attendance data - % records', attendance_count;
    ELSE
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'donations') THEN
        ...
        RAISE NOTICE 'Added donations data - % records', donations_count;
    ELSE
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'member_notes') THEN
        ...
        -- Count member notes records
        SELECT COUNT(*) INTO notes_count 
        FROM member_notes 
        WHERE contact_id IN (contact1_id, contact2_id);
        
        RAISE NOTICE 'Added member notes data - % records', notes_count;
    ELSE
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'follow_ups' AND column_name = 'completed') THEN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'follow_ups' AND column_name = 'scheduled_date') THEN
            ...
            RAISE NOTICE 'Added follow-ups data';
        ELSE
    ...
    RAISE NOTICE 'Donation records: %', donations_count;
    RAISE NOTICE 'Member notes records: %', notes_count;
