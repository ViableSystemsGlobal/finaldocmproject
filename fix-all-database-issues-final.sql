-- FINAL Safe Database Fix Script
-- Run this in your Supabase Dashboard > SQL Editor to fix all console errors
-- This version uses DROP IF EXISTS to safely handle all objects
...
DROP POLICY IF EXISTS "Authenticated users can create messages" ON comms.messages;  
DROP POLICY IF EXISTS "Users can view their own messages" ON comms.messages;
...
-- Create RPC function safely (CREATE OR REPLACE handles existing functions)
