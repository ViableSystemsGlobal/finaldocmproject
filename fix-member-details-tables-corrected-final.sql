-- Fix member details tables to match the code expectations (CORRECTED FINAL VERSION)
...
-- 3. Add sample events (populate ALL required columns: name, event_date, title, start_date)
-- This populates both old columns (name, event_date) and new columns (title, start_date)
INSERT INTO events (name, event_date, title, start_date, event_type, location, description) VALUES
('Sunday Morning Service', NOW() - INTERVAL '7 days', 'Sunday Morning Service', NOW() - INTERVAL '7 days', 'service', 'Main Sanctuary', 'Weekly worship service'),
('Prayer Meeting', NOW() - INTERVAL '3 days', 'Prayer Meeting', NOW() - INTERVAL '3 days', 'meeting', 'Fellowship Hall', 'Midweek prayer gathering'),
('Youth Service', NOW() - INTERVAL '14 days', 'Youth Service', NOW() - INTERVAL '14 days', 'service', 'Youth Hall', 'Special service for young people')
ON CONFLICT DO NOTHING;

-- 4. Disable RLS for development (you can enable it later)
...
SELECT 
    ...
    (SELECT COUNT(*) FROM events WHERE title IS NOT NULL AND name IS NOT NULL) as events_properly_configured; 