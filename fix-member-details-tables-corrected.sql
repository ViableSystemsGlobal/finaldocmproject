-- Fix member details tables to match the code expectations (CORRECTED VERSION)
...
-- 3. Add sample events (populate both name and title to satisfy constraints)
INSERT INTO events (name, title, start_date, event_type, location, description) VALUES
('Sunday Morning Service', 'Sunday Morning Service', NOW() - INTERVAL '7 days', 'service', 'Main Sanctuary', 'Weekly worship service'),
('Prayer Meeting', 'Prayer Meeting', NOW() - INTERVAL '3 days', 'meeting', 'Fellowship Hall', 'Midweek prayer gathering'),
('Youth Service', 'Youth Service', NOW() - INTERVAL '14 days', 'service', 'Youth Hall', 'Special service for young people')
...
SELECT 
    ...
    (SELECT COUNT(*) FROM events WHERE title IS NOT NULL) as events_with_titles; 