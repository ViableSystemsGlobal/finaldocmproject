-- Helper functions for member details queries
...
-- Function to check mobile app user status
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'people' AND table_name = 'mobile_app_users') THEN
    RETURN QUERY
    SELECT mau.id, mau.created_at, mau.last_login_at
    FROM people.mobile_app_users mau
    WHERE mau.contact_id = p_contact_id;
  -- Fall back to public schema
  ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'mobile_app_users') THEN
    RETURN QUERY
    SELECT mau.id, mau.created_at, mau.last_login_at
...
-- Function to get member info from correct schema
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'people' AND table_name = 'members') THEN
    RETURN QUERY
    SELECT m.contact_id, m.joined_at, m.created_at, m.notes
