...
export default function EditCampusPage() {
  ...
  async function loadCampus() {
    try {
      ...
      const { success, data, error } = await fetchCampus(String(id))
      ...
      if (success && data) {
        ...
        toast({
          title: 'Error',
          description: 'Failed to load campus. Please try again.',
          variant: 'destructive',
        })
  ...
  async function onSubmit(data: CampusFormData) {
    try {
      ...
      const { success, data: result, error } = await updateCampus(String(id), data)
      
      if (success) {
        setCampus(result)
        toast({
          title: 'Campus updated',
          description: `Campus "${data.name}" has been updated successfully.`,
        })
        router.push('/settings/campuses')
      } else {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-lg text-slate-600">Loading campus...</span>
        </div>
      </div>
    )
  }
