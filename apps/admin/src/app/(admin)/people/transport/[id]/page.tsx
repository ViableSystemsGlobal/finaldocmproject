...
export default function TransportRequestDetailPage({ params }: { params: { id: string } }) {
  ...
  const [transportRequest, setTransportRequest] = useState<TransportRequestWithRelations | null>(null);
  ...
  useEffect(() => {
    if (isEditMode) {
      router.push(`/people/transport/edit/${id}`);
      return;
    }
  ...
  const handleStatusUpdate = async (status: TransportStatus) => {
    ...
    try {
      const { data, error } = await updateRequest(id, {
        status,
        assigned_driver: status === 'assigned' ? transportRequest.assigned_driver : null,
        assigned_vehicle: status === 'assigned' ? transportRequest.assigned_vehicle : null,
        scheduled_time: status === 'assigned' ? transportRequest.scheduled_time : null,
        notes: status === 'assigned' ? transportRequest.notes : null
      });
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-cyan-50 to-blue-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <Button asChild>
            <Link href="/people/transport">Back to Transport</Link>
          </Button>
        </div>
      </div>
    );
        ...
        <div className="mb-12">
          ...
          {!isEditMode && (
            <div className="flex gap-3 flex-wrap">
              ...
              >
                <Link href={`/people/transport/${id}?mode=edit`}>
        ...
        {isEditMode ? (
          <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <Input
                      ...
                      onChange={(e) => setTransportRequest(prev => ({ ...prev, pickup_address: e.target.value }))}
                  ...
                  <div className="space-y-3">
                    ...
                    <Input
                      id="dropoff_address"
                      value={transportRequest.dropoff_address || ''}
                      onChange={(e) => setTransportRequest(prev => ({ ...prev, dropoff_address: e.target.value }))}
                      disabled={isUpdatingStatus}
              ...
              <div className="p-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  <div className="space-y-3">
                    ...
                    <Select value={transportRequest.status} onValueChange={(value) => handleStatusUpdate(value as TransportStatus)} disabled={isUpdatingStatus}>
                      <SelectTrigger 
                        id="status"
                  ...
                  <div className="space-y-3">
                    ...
                    <Select value={transportRequest.assigned_driver || 'none'} onValueChange={(value) => setTransportRequest(prev => ({ ...prev, assigned_driver: value === 'none' ? null : value }))} disabled={isUpdatingStatus}>
                      <SelectTrigger 
                        id="assigned_driver"
                        className="h-12 border-2 border-slate-200 rounded-xl bg-white/50 focus:border-purple-500 focus:ring-purple-500"
                      >
                  ...
                  <div className="space-y-3">
                    ...
                    <Select value={transportRequest.assigned_vehicle || 'none'} onValueChange={(value) => setTransportRequest(prev => ({ ...prev, assigned_vehicle: value === 'none' ? null : value }))} disabled={isUpdatingStatus}>
                      <SelectTrigger 
                        id="assigned_vehicle"
                        className="h-12 border-2 border-slate-200 rounded-xl bg-white/50 focus:border-purple-500 focus:ring-purple-500"
                      >
                ...
                <div className="space-y-6">
                  <div className="space-y-3">
                    ...
                    <Input
                      id="scheduled_time"
                      type="datetime-local"
                      value={transportRequest.scheduled_time ? transportRequest.scheduled_time.slice(0, 16) : ''}
                      onChange={(e) => setTransportRequest(prev => ({ ...prev, scheduled_time: e.target.value }))}
                      disabled={isUpdatingStatus}
                  ...
                  <div className="space-y-3">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setTransportRequest(prev => ({ ...prev, notes: e.target.value }))}
                      disabled={isUpdatingStatus}
            ...
            <div className="flex justify-end gap-4">
              <Button
                variant="outline"
                onClick={() => router.push(`/people/transport/${id}`)}
                disabled={isUpdatingStatus}
