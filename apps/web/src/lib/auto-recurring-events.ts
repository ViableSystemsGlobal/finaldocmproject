 ...
 * This runs automatically when events are fetched to ensure
 * there's always a future occurrence for recurring events
 */
...
async function generateNextOccurrence(event: RecurringEvent): Promise<boolean> {
  ...
  try {
    ...
    if (event.recurrence_end && nextDate > new Date(event.recurrence_end)) {
      console.log(`Recurrence end reached for event ${event.id}`)
      return false
    }
    ...
    if (event.recurrence_count) {
      ...
      if (count && count >= event.recurrence_count) {
        console.log(`Recurrence count limit reached for event ${event.id}`)
        return false
      }
    }
...
const GENERATION_COOLDOWN = 5000 // 5 seconds cooldown between generations

/**
 * Main function: Auto-generate next occurrences for all past recurring events
 * This should be called when fetching events
 */
export async function autoGenerateRecurringEvents(): Promise<void> {
  ...
  try {
    ...
    // Find all recurring events that have passed AND don't have a future occurrence yet
    // This is more efficient than checking all past events
    const { data: pastRecurringEvents, error } = await supabase
      .from('events')
      .select('*')
    ...
    for (const event of pastRecurringEvents) {
      ...
      const { data: futureOccurrence } = await supabase
        .from('events')
        .select('id')
        .or(`id.eq.${parentId},parent_event_id.eq.${parentId}`)
    ...
    if (eventsToProcess.length === 0) {
      // All recurring events already have future occurrences
      return
    }
